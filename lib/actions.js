...
export const doFetchConferences = (query, reset) => {
  return (dispatch, getState) => {
    const { page, hasEnded, isFetching } = getState().pagination.conference
    if (!hasEnded && !isFetching) {
      dispatch(paginationActions.setFetching('conference', true))
      return confAPI.fetchConferences({ page, query })
        ...
        .then(data => {
          ...
          if (reset) {
            dispatch(confActions.setConferences(data))
          } else {
...
export const doFetchTags = () => {
  return (dispatch, getState) => {
    const { page, hasEnded, isFetching } = getState().pagination.tag
    if (!hasEnded && !isFetching) {
...
export const doFetchSpeakers = () => {
  return (dispatch, getState) => {
    const { page, hasEnded, isFetching } = getState().pagination.speaker
    if (!hasEnded && !isFetching) {
...
export const applyConferenceFilters = () => {
  return (dispatch, getState) => {
    const state = getState()
    const currentRoute = Router.router.route
    const filters = state.pagination.conference.filters
    const validatedFilters = Object.keys(filters).reduce((res, filterKey) => {
      if (filterKey === 'tag') {
        return filters[filterKey].length === 0 ? res : { ...res, tag: filters[filterKey] }
      }
      return filters[filterKey].trim() === '' ? res : { ...res, [filterKey]: filters[filterKey] }
    }, {})
    const queryFilters = queryString.stringify(validatedFilters)
    dispatch(paginationActions.resetPage('conference'))
    Router.push(`${currentRoute}?${queryFilters}`)
    const resetAfterFetch = true
    dispatch(doFetchConferences(validatedFilters, resetAfterFetch))
  }
}
