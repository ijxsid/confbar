...
import { makeQueryString, cleanQuery } from './utils'

export default {
  fetchFeatured: () => {
    return fetch(`${config.backend.base}${config.backend.api}featured/`)
  },
  fetchSearchResults: ({ page, search } = {}) => {
    return fetch(`${config.backend.base}${config.backend.api}search/${makeQueryString({ page, search })}`)
  },
  fetchConferences: ({ page, search, query } = {}) => {
    return fetch(`${config.backend.base}${config.backend.api}conferences/?${cleanQuery({ page, search, ...query })}`)
  },
